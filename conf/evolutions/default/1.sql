...
alter table note add constraint fk_note_user_2 foreign key (user_email) references user (email) on delete restrict on update restrict;
create index ix_note_user_2 on note (user_email);
alter table session add constraint fk_session_user_3 foreign key (user_email) references user (email) on delete restrict on update restrict;
